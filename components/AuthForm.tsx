...
export function AuthForm({ mode }: { mode: "login" | "signup" }) {
  ...
  // Password-less sign-in to the shared read-only demo account.
  async function enterDemo() {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error || "The demo isn't available right now.");
      router.push("/");
      router.refresh();
    } catch (e) {
      setError(e instanceof Error ? e.message : "Something went wrong.");
    ...
    <div className="min-h-screen grid place-items-center px-4 py-10">
      <div className="w-full max-w-sm">
        <div className="flex items-center gap-2 justify-center mb-6">
          ...
          <img src="/logo.png" alt="Gunna" className="w-10 h-10 rounded-xl object-contain" />
          ...
          {!isSignup && (
            <>
              ...
              <button type="button" onClick={enterDemo} disabled={busy}
                className="w-full rounded-lg border border-border py-2.5 text-sm font-medium hover:bg-bg transition-colors disabled:opacity-50">
                Explore the demo
              </button>
              <p className="text-xs text-muted text-center mt-2">
                Real training data, read-only — nothing you do is saved.
              </p>
            </>
          )}
        </div>
